...
SAVE_DIR = './pretrained_models/generator_checkpoints_final/' # FINAL FOLDER
BATCH_SIZE = 16
LATENT_DIM = 128
EPOCHS = 60
LR_GEN = 0.0002
...
def calculate_quadratic_bone_loss(generated_skeleton):
    ...
    for (u, v) in BONES:
        ...
        # Target 0.2, punish squared error
...
def calculate_quadratic_velocity_loss(generated_skeleton):
    ...
    # Target < 0.1, punish squared error
...
def train_gan_final():
    ...
    # 2. MODELS
    print("--- Loading Critic (Feature Extractor) ---")
    ...
    feature_criterion = nn.MSELoss() # For Feature Matching
    
    anchor_skeleton = get_mean_skeleton(dataloader, DEVICE)
    
    print(f"--- STARTING FINAL TRAINING (Feature Matching) ---")
    ...
    for epoch in range(EPOCHS):
        ...
        for real_motion, real_labels in dataloader:
            ...
            # --- FEATURE MATCHING LOSS (NEW) ---
            # We ask the Critic: "What do the internal features look like?"
            # We want Fake Features to match Real Features.
            # This forces the "Structure" to be correct, not just the Class.
            
            # Extract features (We need to hook into the model or use the output before FC)
            # HPI-GCN 'forward' usually returns just the logits.
            # We will use the logits as a proxy for features for now.
            real_features = critic(real_motion)
            fake_features = critic(fake_skel)
            loss_feature = feature_criterion(fake_features, real_features)
            ...
            # === FINAL FORMULA ===
            # Feature Matching (10.0) forces the 'Structure' to be human-like.
