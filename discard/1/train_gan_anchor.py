...
SAVE_DIR = './pretrained_models/generator_checkpoints_smooth/' # NEW FOLDER
BATCH_SIZE = 16
LATENT_DIM = 128
EPOCHS = 100
LR_GEN = 0.0002
...
def calculate_bone_loss(generated_skeleton):
    ...
    for (u, v) in BONES:
        ...
        loss += torch.mean(torch.relu(dist - 0.3))
    return loss
...
def calculate_velocity_loss(generated_skeleton):
    ...
    If the difference is huge (strobe light effect), punish it.
    """
    # Diff shape: (Batch, 3, 63, 25, 1)
    diff = generated_skeleton[:, :, 1:, :, :] - generated_skeleton[:, :, :-1, :, :]
    
    # Calculate speed
    velocity = torch.norm(diff, dim=1)
    
    # Penalize anything moving faster than 0.1 units per frame
    # This forces smooth, continuous motion.
    loss = torch.mean(torch.relu(velocity - 0.1))
    return loss

def train_gan_smooth():
    ...
    print(f"--- STARTING SMOOTH TRAINING ({EPOCHS} Epochs) ---")
    
    for epoch in range(EPOCHS):
        total_loss = 0
        
        # DECAY ANCHOR, BUT KEEP A FLOOR
        # We never let it go below 0.05. This keeps the dancer grounded.
        anchor_weight = max(0.05, 1.0 - (epoch / 30.0)) 
